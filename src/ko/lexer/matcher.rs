...
macro_rules! token {
  ...
  ($tokenizer:expr, $token_type:expr, $accum:expr) => {{
    let tokenizer  = $tokenizer  as &$crate::ko::lexer::tokenizer::Tokenizer<'t>;
    let token_type = $token_type as $crate::ko::lexer::token::TokenType;
    ...
    if TokenType::Text == token_type || TokenType::Char == token_type {
...
impl<'t> Matcher<'t> for StringLiteralMatcher {
  fn try_match(&self, tokenizer: &mut Tokenizer<'t>) -> Result<Option<Token>, ()> {
    ...
    if delimeter == '"' {
      Ok(Some(token!(tokenizer, Text, string)))
    } else {
      if string.len() > 1 {
        let pos = tokenizer.last_position();
